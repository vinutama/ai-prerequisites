...
```bash
.codex/scripts/goal-git.sh config set <goal_source> <target_branch> <platform> <concurrency> <auto_merge>
```
...
Then run `.codex/scripts/goal-git.sh selfcheck` to verify the platform CLI is available and authenticated.
...
```
or `set -a && source .codex/figma.env && set +a && opencode`
