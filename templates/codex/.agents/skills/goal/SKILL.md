...
6. **Plan → Build → Analyze → Review → Loop until done**:
   ...
   - **Multi-repo mode (repos > 1)**:
     ...
     - Spawn the `planner` agent via spawn_agent with agent_type = "planner" ONCE with the full state (all repos, active goal) — planner produces repo-tagged tasks in dependency batches.
     ...
     - After batch: for each repo that had changes, cd into repo, commit, push, create/update PR, spawn the `reviewer` agent via spawn_agent with agent_type = "reviewer" scoped to that repo, run review loop.
...
1. Determine `goal_source`: check if `the arguments from the user message` begins with `--source <type>`. If so, pop both tokens and validate `<type>` is one of `jira|markdown|prompt`. Use it as the effective `goal_source` for this invocation (overrides config). Otherwise read from `.codex/scripts/goal-git.sh config get` (field `goal_source`). If no config exists, treat as `prompt`.
...
4. **Multi-repo orchestration (when repos > 1 in config)**:
   ...
   - Spawn the `planner` agent via spawn_agent with agent_type = "planner" ONCE (planner sees ALL repos, produces repo-tagged tasks in dependency batches).
   ...
   - For each batch in the plan:
     ...
     - After the batch completes:
       - For each repo that had changes in this batch:
         ...
         - Spawn the `reviewer` agent via spawn_agent with agent_type = "reviewer" scoped to that repo (pass repo_path).
