...
`.qoder/goal-config.json` (project-level, gitignored as part of `.qoder/`).
Figma PAT is stored in `.qoder/figma.env` (gitignored as part of `.qoder/`).
...
into `.qoder/skills/` (project-level, filtered by category and risk), and
...
Do not use `@mentions` or manually read `.qoder/skills/*/SKILL.md`.
...
- `.qoder/` — entire directory (agents, commands, skills, scripts, config, secrets). Gitignored — generated by `init.sh`, never committed.
...
- `.qoder/settings.json` — Figma MCP block (when enabled). Project-level, created by /init-goal.
...
`.qoder/scripts/goal-git.sh state` — never from a global or cwd-relative path.

### Git workflow
NEVER invoke `git`, `gh`, or `glab` directly. ALL git and state operations
MUST go through `.qoder/scripts/goal-git.sh`:
```bash
.qoder/scripts/goal-git.sh start <goal> [ticket] [task_type]  # create branch (jira: task_type/TICKET-slug)
.qoder/scripts/goal-git.sh continue [id]     # resume goal by branch/text
...
.qoder/scripts/goal-git.sh push               # push to origin
.qoder/scripts/goal-git.sh pr                 # create/update PR
.qoder/scripts/goal-git.sh pending            # check unresolved PR threads
...
.qoder/scripts/goal-git.sh resolve <thread-id>  # resolve a thread
...
.qoder/scripts/goal-git.sh review list [repo_path]
.qoder/scripts/goal-git.sh review resolve <id> [repo_path]
.qoder/scripts/goal-git.sh review pending [repo_path]  # local review gate
.qoder/scripts/goal-git.sh review iterate [repo_path]
...
.qoder/scripts/goal-git.sh worktree merge <slug>  # merge worktree into goal branch
...
.qoder/scripts/goal-git.sh figma setup <token>   # store PAT + enable Figma MCP
...
- After code changes, run `.qoder/scripts/goal-git.sh analyze` (gitnexus + rtk gain).
  If it fails, STOP.
...
- Run `.qoder/scripts/goal-git.sh pending` (inline) or `review pending` (local) to check review status.
...
- When `auto_merge` is `true`, orchestrator runs `.qoder/scripts/goal-git.sh merge` after
...
`.qoder/figma.env` and a default design link in `goal-config.json`:
`figma_design_url`, `figma_file_key`, `figma_node_id`. Planner, builder, and
visual-reviewer consult Figma for UI work. Use `run-qoder.sh` to load secrets.

### UI/UX Pro Max (optional)
Install via `/init-skills` question **UI/UX Pro Max**
(`npx -y ui-ux-pro-max-cli init --ai qoder` → `.qoder/skills/ui-ux-pro-max/`).
...
- **No Figma** — planner generates/reuses `design-system/MASTER.md` via
  `python3 .qoder/skills/ui-ux-pro-max/scripts/search.py ... --design-system --persist`.
