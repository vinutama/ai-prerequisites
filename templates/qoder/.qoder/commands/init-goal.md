...
1. **Goal source** — Where will goals come from?
   ...
   **If the user selects `issues`:** before continuing, verify the forge CLI can read the list:
   ...
   - Run: `.qoder/scripts/goal-git.sh issues list "<url>" 1`
...
4. **Repo selection** — (multi-repo only) Which repositories should this goal workflow cover?
   ...
   - **Multi-repo:** Show the detected repos above. Ask the user:
     ...
     *"Which repos should goals cover? (all / comma-separated names)"*
     ...
     ```bash
     jq --argjson repos '["repo1","repo2"]' '.repos = $repos' .qoder/goal-config.json > .qoder/goal-config.json.tmp && mv .qoder/goal-config.json.tmp .qoder/goal-config.json
     ```

     Confirm: `jq '.repos' .qoder/goal-config.json`
...
6. **Figma design lookup** — Connect Figma to look up preferred designs during UI goals?
   ...
   **6b (only if yes):** Figma Personal Access Token
   ...
   - Warn: token is stored in `.qoder/figma.env` (project-level, gitignored)
   - Run: `.qoder/scripts/goal-git.sh figma setup "<token>"`
   - Optionally verify after sourcing env: `set -a && source .qoder/figma.env && set +a && claude && claude  qoder mcp list`
   ...
   **6c (only if yes, after token saved):** Default Figma design link
   ...
   - Run: `.qoder/scripts/goal-git.sh figma design set "<url>"`
...
```bash
.qoder/scripts/goal-git.sh config set <goal_source> <target_branch> <platform> <concurrency> <auto_merge> <review_mode> <review_max_iterations>
```
...
jq --arg url "<issue_list_url>" --argjson limit <issue_limit> \
  ...
  .qoder/goal-config.json > .qoder/goal-config.json.tmp && mv .qoder/goal-config.json.tmp .qoder/goal-config.json
```
...
```bash
.qoder/scripts/goal-git.sh config get
...
```
or `set -a && source .qoder/figma.env && set +a && qoder`

Only use `.qoder/scripts/goal-git.sh` for all git and state operations — never run `git`, `gh`, or `glab` directly.
