...
3. Fetch issues:
   ```bash
   .qoder/scripts/goal-git.sh issues list "<url>" <count>
   ```
...
2. Otherwise take the first token as a candidate identifier. Check it against
   existing goals via `.qoder/scripts/goal-git.sh list` (branch or goal text match).
...
3. Run `.qoder/scripts/goal-git.sh continue "<identifier>"` (empty identifier = active goal).
4. **Issue run resume:** If `.qoder/scripts/goal-git.sh issues queue` returns a non-empty array with any entry where `status` is not `completed`, resume the **issue queue** for that `run_id`:
...
7. **Plan → Build → Analyze → Review → Loop until done**:
   ...
   - **Multi-repo mode (repos > 1)**:
     ...
     - Read full `state.json` (`.qoder/scripts/goal-git.sh state`).
...
2. Resolve the goal text based on source:
   ...
   - `jira` — resolve ticket + task_type, then fetch the issue:
     ...
     5. Call start with ticket and type:
        ```bash
        GOAL_SOURCE_OVERRIDE=<effective_source> .qoder/scripts/goal-git.sh start "<resolved goal>" "<ticket-key>" "<task_type>"
        ```
        Branch becomes `{task_type}/{TICKET}-{slug}` (e.g. `feat/DEL-4123-add-health-check`).
3. For `prompt` and `markdown` only: run `GOAL_SOURCE_OVERRIDE=<effective_source> .qoder/scripts/goal-git.sh start "<resolved goal>"` (branch `goal/<slug>`).
   For `jira`, start was already called in step 2.
4. **Multi-repo orchestration (when repos > 1 in config)**:
   - Read repos from `state.json` (`.qoder/scripts/goal-git.sh state | jq '.repos'`).
