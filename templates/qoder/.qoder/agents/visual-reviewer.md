...
This agent requires a vision-capable model (inherit tier with multimodal
capabilities per `.qoder/goal-models.json`).
...
When `figma_enabled` is true in `.qoder/scripts/goal-git.sh config get`, compare
...
3. Run `.qoder/scripts/goal-git.sh diff` to see all frontend changes against the base branch.
...
8. **Post inline comments** for each new visual issue:
   ...
   ```
9. Run `.qoder/scripts/goal-git.sh pending` and `.qoder/scripts/goal-git.sh threads`.
