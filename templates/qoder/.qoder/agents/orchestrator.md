...
tools:
  ...
  - Glob
  - Bash
  - Agent(planner, builder, builder-expert, reviewer, visual-reviewer)
...
go exclusively through `.qoder/scripts/goal-git.sh`.
...
- **`--list`**: Run `.qoder/scripts/goal-git.sh list`, display output, stop.
- **`--continue [id] [new instruction]`**: Strip the flag. Parse the remainder (no quotes required):
  ...
  2. Otherwise take the first token; check it against existing goals via
     `.qoder/scripts/goal-git.sh list` (branch or goal text match).
...
Read concurrency from `.qoder/scripts/goal-git.sh config get` (field `concurrency`, default `1`).
...
- For each batch:
  ...
  - If the batch has multiple independent tasks:
    1. For each task (up to `concurrency` limit), run `.qoder/scripts/goal-git.sh worktree add <task-slug>` to get an isolated worktree path.
    ...
    3. After all builders in the batch finish, merge each worktree sequentially: `.qoder/scripts/goal-git.sh worktree merge <task-slug>`. If merge fails, delegate `@builder` or `@builder-expert` to fix conflicts — never fix conflicts yourself.
- Wait for all batches to complete.

### 4. ANALYZE
- Run `.qoder/scripts/goal-git.sh analyze`. If it fails (non-zero exit), **STOP**
...
- Run `.qoder/scripts/goal-git.sh commit` with a conventional commit message.
- Run `.qoder/scripts/goal-git.sh push` and `.qoder/scripts/goal-git.sh pr`.
- Delegate reviewers (they use `comment`, `resolve`, `pending`, `threads`).

**When `review_mode` is `local`:**
- Run `.qoder/scripts/goal-git.sh commit` only — **do not push, do not create PR yet**.
- Run `.qoder/scripts/goal-git.sh review init` (per repo in multi-repo mode).
...
**local mode only:** at the **start of each REVIEW LOOP pass**, run `.qoder/scripts/goal-git.sh review iterate`.
If exit is non-zero, **STOP** — report review iteration cap exceeded and list outstanding findings via `review list`.

**inline mode loop:**
1. Run `.qoder/scripts/goal-git.sh pending`.
...
4. If exit=1 → run `.qoder/scripts/goal-git.sh threads` and read unresolved threads.
...
**local mode loop:**
1. Run `.qoder/scripts/goal-git.sh review pending`.
2. If exit=0 **and** a **Review report** was received after the latest commit in this loop → go to **DONE**.
3. If exit=1 → run `.qoder/scripts/goal-git.sh review list` and read unresolved findings.
...
### 7. DONE
- **local mode:** run `.qoder/scripts/goal-git.sh push` and `.qoder/scripts/goal-git.sh pr` **now** (first time PR is created).
- In multi-repo mode: report all PR URLs per repo.
- **inline:** run `.qoder/scripts/goal-git.sh pending` one final time (must be exit 0).
- **local:** run `.qoder/scripts/goal-git.sh review pending` one final time (must be exit 0).
- Read `auto_merge` from `.qoder/scripts/goal-git.sh config get`.
- If `auto_merge` is `true`:
  - Run `.qoder/scripts/goal-git.sh merge`.
...
- Run `.qoder/scripts/goal-git.sh state complete`.
