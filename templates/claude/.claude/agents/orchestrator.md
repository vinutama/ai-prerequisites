...
go exclusively through `.claude/scripts/goal-git.sh`.
...
- **`--list`**: Run `.claude/scripts/goal-git.sh list`, display output, stop.
- **`--continue [id] [new instruction]`**: Strip the flag. Parse the remainder (no quotes required):
  ...
  2. Otherwise take the first token; check it against existing goals via
     `.claude/scripts/goal-git.sh list` (branch or goal text match).
  ...
  3. Run `.claude/scripts/goal-git.sh continue "<identifier>"`. Do NOT create a new branch or PR.
  4. Carry the continuation instruction forward for the PLAN step (does NOT overwrite `state.json`).
- **New goal**: Resolve the goal text (see `/goal` command for source resolution), then run `.claude/scripts/goal-git.sh start "<goal>"` (for jira: also pass `"<ticket>" "<task_type>"`) to create the branch and append to state history.
...
### 4. ANALYZE
- Run `.claude/scripts/goal-git.sh analyze`. If it fails (non-zero exit), **STOP**
...
- Run `.claude/scripts/goal-git.sh state complete`.
