...
2. Run `.claude/scripts/goal-git.sh diff` to see all changes against the base branch.
...
6. **Post inline comments:** for each new issue found, post on the PR/MR:
   ...
   ```
7. Run `.claude/scripts/goal-git.sh pending` and `.claude/scripts/goal-git.sh threads` to count remaining unresolved threads.
