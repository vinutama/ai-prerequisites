...
into `.claude/skills/` at the **project level** (not global `~/.agents/skills`),
...
1. **Install mode** — Which install preset?
   ...
   ```bash
   npx agentic-awesome-skills --path .claude/skills \
...
```bash
npx agentic-awesome-skills --path .claude/skills --category <categories> --risk <risk>
```
Add `--tags <tags>` only if the user provided tags.

**After agentic-awesome-skills install:**
1. List installed skills: `ls .claude/skills/*/SKILL.md`
2. Update `CLAUDE.md` — find or create the `## Available skills` section and append
...
5. **UI/UX Pro Max** — Install design intelligence skill for UI/UX/frontend tasks
   ...
   - `yes` — continue:
     ...
     2. Install from the project root:
        ...
        ```
     3. Verify: `ls .claude/skills/ui-ux-pro-max/`
     4. Append `ui-ux-pro-max` to `CLAUDE.md` `## Available skills` if not already listed:
...
installed, and how to invoke (e.g. `the Skill tool (`ui-ux-pro-max`)`).

**Important:**
- Skills are loaded via the Skill tool; do NOT load all SKILL.md files into context
