...
### `/goal --list`
Run `.claude/scripts/goal-git.sh list` and display the output.
If the state has `repos` with more than one entry, also show each repo path and its active branch from `state.json` (run `.claude/scripts/goal-git.sh state | jq '.repos'`). Stop.
...
3. Fetch issues:
   ```bash
   .claude/scripts/goal-git.sh issues list "<url>" <count>
   ```
...
2. Otherwise take the first token as a candidate identifier. Check it against
   existing goals via `.claude/scripts/goal-git.sh list` (branch or goal text match).
...
3. Run `.claude/scripts/goal-git.sh continue "<identifier>"` (empty identifier = active goal).
4. **Issue run resume:** If `.claude/scripts/goal-git.sh issues queue` returns a non-empty array with any entry where `status` is not `completed`, resume the **issue queue** for that `run_id`:
...
7. **Plan → Build → Analyze → Review → Loop until done**:
   ...
   - **Multi-repo mode (repos > 1)**:
     ...
     - Read full `state.json` (`.claude/scripts/goal-git.sh state`).
...
1. Determine `goal_source`: check if `$ARGUMENTS` begins with `--source <type>`. If so, pop both tokens and validate `<type>` is one of `jira|markdown|prompt|issues`. Use it as the effective `goal_source` for this invocation (overrides config). Otherwise read from `.claude/scripts/goal-git.sh config get` (field `goal_source`). If no config exists, treat as `prompt`.
   ...
   Prefix all `goal-git.sh start` calls with `GOAL_SOURCE_OVERRIDE=<effective_source>` (e.g. `GOAL_SOURCE_OVERRIDE=prompt .claude/scripts/goal-git.sh start ...`).
2. Resolve the goal text based on source:
   ...
   - `jira` — resolve ticket + task_type, then fetch the issue:
     ...
     5. Call start with ticket and type:
        ```bash
        GOAL_SOURCE_OVERRIDE=<effective_source> .claude/scripts/goal-git.sh start "<resolved goal>" "<ticket-key>" "<task_type>"
        ```
        Branch becomes `{task_type}/{TICKET}-{slug}` (e.g. `feat/DEL-4123-add-health-check`).
3. For `prompt` and `markdown` only: run `GOAL_SOURCE_OVERRIDE=<effective_source> .claude/scripts/goal-git.sh start "<resolved goal>"` (branch `goal/<slug>`).
   For `jira`, start was already called in step 2.
4. **Multi-repo orchestration (when repos > 1 in config)**:
   - Read repos from `state.json` (`.claude/scripts/goal-git.sh state | jq '.repos'`).
