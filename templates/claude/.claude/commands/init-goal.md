...
```bash
.claude/scripts/goal-git.sh config set <goal_source> <target_branch> <platform> <concurrency> <auto_merge> <review_mode> <review_max_iterations>
```
...
jq --arg url "<issue_list_url>" --argjson limit <issue_limit> \
  ...
  .claude/goal-config.json > .claude/goal-config.json.tmp && mv .claude/goal-config.json.tmp .claude/goal-config.json
```
