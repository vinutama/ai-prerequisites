...
`init.sh` syncs `model` into each agent `.md` and generates `.mcp.json`.
...
Model fallbacks are configured in project-level `.mcp.json` (plugin +
per-agent `fallback_models`, generated from `.claude/goal-models.json` by
`init.sh`) and `.claude/opencode-model-fallback.json`. On rate limit or API
...
- `.mcp.json` — model fallback config + Figma MCP block. Project-level, generated by init.sh / init-goal.
...
.claude/scripts/goal-git.sh review init [repo_path]  # init local findings file
.claude/scripts/goal-git.sh review add <path> <line> <severity> <body> [repo_path]
.claude/scripts/goal-git.sh review list [repo_path]
.claude/scripts/goal-git.sh review resolve <id> [repo_path]
.claude/scripts/goal-git.sh review pending [repo_path]  # local review gate
.claude/scripts/goal-git.sh review iterate [repo_path]
...
.claude/scripts/goal-git.sh issues list [url] [limit]  # list open issues from forge URL
.claude/scripts/goal-git.sh issues start <n> [--worktree]  # start issue goal (branch off base)
.claude/scripts/goal-git.sh issues queue  # current run's issue entries
.claude/scripts/goal-git.sh issues finish <n>  # complete issue + remove worktree
...
```bash
.claude/scripts/run-opencode.sh
```
...
- Run `.claude/scripts/goal-git.sh pending` (inline) or `review pending` (local) to check review status.
...
(`npx -y ui-ux-pro-max-cli init --ai opencode` → `.claude/skills/ui-ux-pro-max/`).
