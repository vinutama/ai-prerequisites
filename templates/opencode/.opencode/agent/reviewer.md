...
3. Run `.opencode/scripts/goal-git.sh diff` to see all changes against the base branch.
...
7. **Post inline comments** for each new issue:
   ...
   ```
8. Run `.opencode/scripts/goal-git.sh pending` and `.opencode/scripts/goal-git.sh threads`.
