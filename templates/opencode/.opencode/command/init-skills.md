...
5. **UI/UX Pro Max** — Install design intelligence skill for UI/UX/frontend tasks
   ...
   - `yes` — continue:
     ...
     2. Install from the project root:
        ```bash
        npx -y ui-ux-pro-max-cli init --ai opencode
        ```
     3. Verify: `ls .opencode/skills/ui-ux-pro-max/`
...
installed, and how to invoke (e.g. `skill({ name: "ui-ux-pro-max" })`).

**Important:**
- Skills are loaded via OpenCode's native `skill` tool
  (`skill({ name: "<skill-name>" })`); do NOT load all SKILL.md files into context
