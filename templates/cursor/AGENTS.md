...
into `.cursor/skills/` (project-level, filtered by category and risk), and
...
| Agent | Role | Model |
|---|---|---|
| `orchestrator` | Manages the full loop | inherit |
| `planner` | Architecture & plans — tags tasks @builder or @builder-expert | inherit |
| `builder` | Routine execution (CRUD, UI, refactors, config, tests) | inherit |
| `builder-expert` | Complex execution (algorithms, concurrency, security, perf) | inherit |
| `reviewer` | Code review + inline PR comments | inherit |
...
`init.sh` syncs `model` and `readonly` into each agent `.md`. Cursor has no closed model-id enum; agents inherit the session model.
...
Cursor has no model-fallback plugin; each agent inherits the session model (`model: inherit`). `orchestrator`, `reviewer`, and `visual-reviewer` must not edit application source — that rule is prompt-enforced (`readonly` would block `goal-git.sh` writes).
...
- `.cursor/mcp.json` — Figma MCP block (when enabled). Generated by init-goal.
...
- After code changes, run `.cursor/scripts/goal-git.sh analyze` (gitnexus + rtk gain).
  If it fails, STOP.
...
- Run `.cursor/scripts/goal-git.sh pending` to check PR threads.
...
- When `auto_merge` is `true`, orchestrator runs `.cursor/scripts/goal-git.sh merge` after
...
(`npx -y ui-ux-pro-max-cli init --ai cursor` → `.cursor/skills/ui-ux-pro-max/`).
