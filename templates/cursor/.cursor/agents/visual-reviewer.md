...
This agent requires a vision-capable model (`inherit` — session model must be vision-capable per
...
- If `webapp-testing` is available via `/skill-name`, load it and prefer capturing a screenshot before visual review.
...
Regardless of Figma, if `ui-ux-pro-max` is available via `/skill-name`, load it and
...
2. Run `.cursor/scripts/goal-git.sh diff` to see all frontend changes against the base branch.
...
7. **Post inline comments:** for each new visual issue found:
   ...
   ```
8. Run `.cursor/scripts/goal-git.sh pending` and `.cursor/scripts/goal-git.sh threads` to count remaining unresolved threads.
