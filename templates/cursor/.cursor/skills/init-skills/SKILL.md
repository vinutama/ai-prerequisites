...
name: init-skills
description: >-
  Install only the agentic-awesome-skills attached to Cursor goal-loop agents,
...
into `.cursor/skills/` at the **project level** (not global `~/.agents/skills`),
...
Read each `.cursor/agents/<role>.md` `## Related skills` section if present.
...
2. **Agents** (custom only) — Which agents' skills to install? (multi-select)
   ...
   ```bash
   npx agentic-awesome-skills --path .cursor/skills --skills <comma-separated-skill-names>
   ```
   Do **not** pass `--category`.

**After agentic-awesome-skills install:**
1. List installed skills: `ls .cursor/skills/*/SKILL.md`
2. Update `AGENTS.md` — find or create the `## Available skills` section and append
   ...
   Skip `goal-loop` / `goal` / `init-goal` / `init-skills` / `create-issues` (already present). Do not duplicate existing entries.
...
3. **UI/UX Pro Max** — Install design intelligence skill for UI/UX/frontend tasks
   ...
   - `yes` — continue:
     ...
     2. Install from the project root:
        ...
        ```
     3. Verify: `ls .cursor/skills/ui-ux-pro-max/`
...
- Re-running `/init-skills` merges into `.cursor/skills/` (exact `--skills` set is managed).
...
- If agent markdown files gain or drop related skills later, update this map to match before re-running.
